# INFORME FINAL: Bot Autónomo para Videojuego usando Visión por Computador

**Proyecto Final - Ingeniería de Sistemas**  
**Periodo: 202610**  
**Docente asesor: Margarita Gamarra**

---

## 1. Introducción

La automatización de videojuegos mediante visión por computador representa un desafío interdisciplinario que integra conceptos de procesamiento digital de imágenes, percepción visual, sistemas autónomos y control en tiempo real. En la mayoría de videojuegos modernos, la toma de decisiones ocurre en tiempo real y está fundamentada únicamente en información visual presentada en pantalla. Automatizar la ejecución de un videojuego sin acceso interno al motor del juego representa un desafío técnico significativo, ya que el sistema debe interpretar el estado del juego exclusivamente a partir de los píxeles capturados en cada frame.

En los últimos años, el acceso a herramientas de visión por computador clásica y el aumento en disponibilidad de recursos computacionales ha permitido que pequeños equipos de desarrollo implementen soluciones de automatización visual de complejidad moderada. Sin embargo, estas soluciones enfrentan limitaciones bien documentadas en la literatura: sensibilidad a variaciones en la escena (iluminación, fondos complejos, oclusiones), latencia acumulada en el pipeline percepción-decisión-acción, y la necesidad de calibración manual para cada contexto específico.

Este proyecto propone el **diseño e implementación de un bot autónomo capaz de jugar el videojuego móvil Banana Kong** en tiempo real. El sistema opera bajo un enfoque **black-box visual** (sin acceso a memoria interna, sin modificación del cliente del juego), utilizando:

- **Captura de pantalla en tiempo real** del emulador mediante detección de ventana por nombre
- **Técnicas clásicas de visión por computador** (segmentación HSV, análisis de contornos, template matching) para detectar elementos relevantes
- **Sistema de reglas predefinidas** para traducir el estado percibido en decisiones concretas
- **Simulación de controles** (mouse) para ejecutar acciones en el emulador

El videojuego seleccionado es **Banana Kong**, un juego de plataformas con desplazamiento lateral continuo que presenta características ideales para validación: escena visualmente compleja con fondo dinámico, múltiples elementos simultáneos de colores y formas diferenciables, variaciones de iluminación por zonas, y una dinámica que exige reacción en tiempo real. Además, sus métricas de desempeño (puntaje, distancia recorrida, nivel completado) son objetivas, numéricas y reproducibles.
...
Por tanto, Banana Kong constituye un entorno adecuado para evaluar visión por computador clásica aplicada a control autónomo, ya que combina alta frecuencia de eventos, incertidumbre visual y necesidad de respuesta en ventanas temporales reducidas.
...
La visión por computador clásica ha sido durante décadas el enfoque predominante para sistemas de percepción visual en tiempo real, especialmente en robótica industrial y automatización. A diferencia de los modelos de aprendizaje profundo, no requiere conjuntos de datos etiquetados masivos ni entrenamiento previo, lo que la hace atractiva para entornos con recursos computacionales limitados o donde se necesita una solución interpretable y ajustable rápidamente.

Sin embargo, su principal limitación está bien documentada: la sensibilidad a variaciones en la escena. Cambios de iluminación, fondos complejos, oclusiones parciales y objetos en movimiento pueden degradar significativamente la precisión de detección. Evaluar estos límites en condiciones reales y controladas es relevante para determinar en qué contextos este enfoque es suficiente y en cuáles se requiere una alternativa más robusta (redes neuronales, aprendizaje por refuerzo, etc.).
...
El videojuego **Banana Kong** se utiliza en este proyecto como entorno de evaluación controlado. Sus características lo hacen adecuado:

- **Escena visualmente compleja:** Fondo dinámico, múltiples elementos simultáneos, variaciones de iluminación
- **Elementos diferenciables:** Elementos del juego presentan colores y formas distintivas
- **Dinámica de tiempo real:** Exige toma de decisiones rápida y reacción oportuna
- **Métricas objetivas:** Puntaje, distancia recorrida, niveles completados son reproducibles y medibles

#### 3.1.2 Pregunta Central de Investigación
...
- **Percepción visual:** Identificar y localizar elementos de interés en la escena usando técnicas clásicas, sin recurrir a redes neuronales
- **Representación del entorno:** Construir una descripción estructurada del estado del juego a partir de las detecciones
- **Decisión basada en reglas:** Diseñar un conjunto de reglas que traduzca estado percibido en acciones efectivas y oportunas
- **Control del agente:** Simular entradas de mouse sobre el emulador con precisión y sincronización

#### 3.1.4 Preguntas de Investigación Específicas

1. ¿Qué tan precisas y estables son las técnicas clásicas de visión por computador para detectar elementos en una escena con fondo dinámico, iluminación variable y objetos en movimiento?
2. ¿Es suficiente un sistema de reglas predefinidas para generar comportamiento autónomo efectivo cuando la información proviene únicamente de detección visual clásica?
3. ¿Cómo afecta la latencia acumulada del pipeline percepción–decisión–acción al desempeño del agente en un entorno que exige reacción en tiempo real?
4. ¿Cuáles son las principales fuentes de fallo del sistema y cómo se manifiestan en el desempeño medido por puntaje?
...
- La interacción será **exclusivamente mediante captura de pantalla** (mss) y **simulación de teclado** (pyautogui)
- Debe cumplir **restricciones de tiempo real**, minimizando latencia entre percepción y acción
- Se trabajará con **resolución fija de 960×540** píxeles en emulador MuMu Player
- La detección se basa **exclusivamente en técnicas clásicas de OpenCV**, sin redes neuronales

#### 3.2.2 Restricciones Operativas y Éticas

- El juego seleccionado es **offline sin sistemas anti-cheat activos**
- No se utilizarán juegos online competitivos
- Se respetan los términos de servicio del juego
- El bot opera únicamente sobre la instancia local del emulador

#### 3.2.3 Supuestos de Diseño

- El videojuego mantiene una estructura visual **relativamente consistente entre sesiones**
- Los elementos del juego presentan **características visuales suficientemente diferenciables** mediante técnicas clásicas
- La resolución del emulador se mantiene **fija en 960×540** durante ejecución
- El emulador MuMu Player se ejecuta con el nombre de ventana "Android Device"
- Los colores de los elementos **no varían significativamente** entre dispositivos

### 3.3 Alcance

#### 3.3.1 Incluye

- Captura automática de pantalla del emulador con detección de ventana por nombre
- Preprocesamiento de imagen y aplicación de técnicas clásicas de visión por computador
- **Detección de elementos:** Bananas, obstáculos (troncos, arbustos, aviones, paredes, rocas, cuevas, totems, tubos), plataformas, plataformas de madera, Kong, agua, barras potenciadoras
- Módulo de decisión basado en **reglas predefinidas** según el estado detectado
- Módulo de acción para generación de entradas simuladas: salto (C), planeo (Space), bajada (flecha abajo), dash
- Visualización de debug en tiempo real con rectángulos de color por tipo de elemento
- Arquitectura modular con separación entre percepción, decisión y acción
- Configuración centralizada de parámetros de detección
...
- Uso de redes neuronales (YOLO, CNN, segmentación profunda)
- Jugabilidad en mundos alternativos del juego
- Gestión de mejoras del personaje ni interacción con menús
- Generalización automática a otras resoluciones u otros videojuegos
...
Diseñar e implementar un **sistema autónomo que juegue el videojuego móvil Banana Kong en tiempo real** mediante el procesamiento de información visual capturada de la pantalla, tomando decisiones basadas en **reglas predefinidas** y ejecutando acciones a través de **simulación de controles de mouse**, con el propósito de **maximizar el puntaje obtenido** como indicador principal de desempeño.

### 4.2 Objetivos Específicos

1. **OE1:** Implementar un módulo de captura de pantalla en tiempo real que se adapte automáticamente a la posición y estado de la ventana del emulador

2. **OE2:** Desarrollar un pipeline de visión por computador clásica capaz de detectar **al menos 13 tipos de elementos** del juego (bananas, 8 tipos de obstáculos, plataformas, agua, Kong) mediante técnicas apropiadas según las características visuales de cada elemento

3. **OE3:** Construir un módulo de decisión basado en reglas que determine la acción óptima (saltar, planear, bajar, dash) a partir del estado visual detectado en tiempo real

4. **OE4:** Implementar un módulo de control que traduzca decisiones en entradas de mouse simuladas sobre el emulador con precisión y sincronización

5. **OE5:** Integrar un sistema de métricas que registre desempeño (puntaje, distancia, nivel, tasa de éxito en detecciones) para validar y analizar el funcionamiento del bot

6. **OE6:** Validar el sistema mediante pruebas experimentales que demuestren la viabilidad de la solución y caractericen sus limitaciones
...
**Descripción:** Herramienta de automatización ampliamente utilizada para automatizar acciones repetitivas en interfaces gráficas y videojuegos.

**Enfoque:** Puramente reactivo. Detecta un píxel específico en una posición predefinida; cuando alcanza un color determinado, ejecuta una acción (tecla, clic).

**Limitaciones:**
- Depende de coordenadas fijas en pantalla
- No construye representación del entorno
- Frágil ante cambios de resolución, escala o posición de ventana
- No detecta múltiples instancias de un elemento
- No puede inferir distancias relativas entre objetos

**Aplicabilidad al proyecto:** Inapropiado para Banana Kong; no maneja la complejidad de múltiples elementos dinámicos.

#### SikuliX (antiguamente Sikuli)

**Descripción:** Permite automatizar interfaces gráficas mediante reconocimiento visual de imágenes capturadas como referencia.

**Enfoque:** Template matching estático. Captura una imagen de referencia (screenshot) y busca esa imagen exacta dentro de la pantalla del juego.

**Limitaciones:**
- Depende de imágenes de referencia estáticas
- Sensible a cambios gráficos del juego (skins, efectos visuales)
- Requiere capturar templates para cada variante visual
- No es escalable a elementos dinámicos

**Aplicabilidad al proyecto:** Limitada; Banana Kong tiene variabilidad visual considerable.

### 5.2 Soluciones Open-Source

#### Bot Chrome Dino

**Descripción:** Automatiza el juego Chrome Dino (que aparece cuando el navegador pierde conexión a internet).

**Arquitectura:** Captura (mss) → Percepción (OpenCV) → Decisión (reglas simples) → Acción (pyautogui)

**Detección:** Busca obstáculos (cactus) por análisis de bordes y contornos. Fondo blanco uniforme simplifica enormemente el problema de segmentación.

**Decisión:** Saltar si hay obstáculo detectado.

**Logros:** Desempeño excelente (~10k+ puntos) debido a la simplicidad visual del juego.

**Limitaciones:**
- Fondo blanco uniforme (no generalizable a escenas complejas)
- Juego con dinámica muy simple
- No maneja múltiples tipos de obstáculos

#### SerpentAI

**Descripción:** Framework genérico para bots de videojuegos con soporte modular.

**Arquitectura:** Captura → Percepción (color, template, ML) → Decisión (reglas, modelo) → Acción

**Aplicaciones documentadas:** Shovel Knight (plataformas), otros juegos indie.

**Enfoque:** Flexible, soporta múltiples técnicas de detección.

**Limitaciones:**
- No está optimizado para escenas con fondos complejos y variables
- Requiere personalización extensiva por juego
- Documentación limitada para casos complejos

#### MAA (Múltiples Acciones Automatizadas)

**Descripción:** Automatiza el juego móvil Arknights en emulador Android.

**Arquitectura:** Captura (emulador) → Percepción (OpenCV, templates) → Decisión (reglas, scripts) → Acción (toque, swipe)

**Características:** Detecta interfaces, menús, unidades; ejecuta estrategias de batalla complejas.

**Desempeño:** 10–30 acciones por segundo, tasas de éxito ~95% en tareas definidas.

**Limitaciones:**
- Construido sobre templates estáticos (sensible a actualizaciones visuales del juego)
- Estrategias manual-coding específicas de Arknights
- No es fácilmente generalizable a otros juegos

### 5.3 Análisis Comparativo

| Criterio | AutoHotkey | SikuliX | Chrome Dino | SerpentAI | MAA | Este Proyecto |
|----------|-----------|---------|------------|-----------|-----|-------------|
| **Complejidad escena** | Muy simple | Simple | Muy simple | Moderada | Compleja | **Compleja** |
| **Fondo dinámico** | No | No | No | Parcial | Parcial | **Sí** |
| **Múltiples objetos** | No | Parcial | No | Sí | Sí | **Sí (13+)** |
| **Tiempo real** | Sí | Parcial | Sí | Sí | Sí | **Sí** |
| **Técnica percepción** | Píxel fijo | Template | Bordes | Mixta | Template+OpenCV | **OpenCV clásico** |
| **Módulo decisión** | Trivial | Trivial | Regla simple | Flexible | Reglas+Scripts | **Reglas estructuradas** |
| **Precisión en fondos variables** | N/A | Baja | N/A | Media | Media | **Por validar** |

**Vacío identificado:** No existe documentación de implementación de bot de videojuego con **fondo dinámico de alta variabilidad**, **múltiples tipos de elementos simultáneos** y **técnicas clásicas de visión por computador** que mantuviera tiempo real en hardware de propósito general. Este proyecto aborda este vacío.
...
### 6.3 Requerimientos Funcionales (Detalle RF)

#### RF1: Captura de Pantalla
- **RF1.1:** El sistema debe capturar frames del emulador a una tasa de al menos 30 FPS
- **RF1.2:** La captura debe adaptarse automáticamente a la posición y tamaño de la ventana del emulador
- **RF1.3:** La latencia de captura no debe exceder 30 ms

#### RF2: Detección de Elementos
- **RF2.1:** El sistema debe detectar bananas con una precisión mínima del 85%
- **RF2.2:** El sistema debe detectar obstáculos (troncos, arbustos, aviones, paredes, rocas, cuevas, totems, tubos) con precisión mínima del 80%
- **RF2.3:** El sistema debe detectar la plataforma actual de Kong con precisión mínima del 90%
- **RF2.4:** El sistema debe detectar agua (peligro) con precisión mínima del 85%
- **RF2.5:** El sistema debe localizar a Kong (posición x, y) con error máximo de ±15 píxeles

#### RF3: Módulo de Decisión
- **RF3.1:** El sistema debe evaluar el estado del juego y determinar la acción óptima cada frame (≤30 ms)
- **RF3.2:** El módulo debe implementar al menos 5 reglas que cubran escenarios básicos (evitar obstáculos, recoger bananas, evitar agua, usar plataformas)
- **RF3.3:** Las reglas deben aplicarse con una latencia máxima de 10 ms

#### RF4: Generación de Acciones
- **RF4.1:** El sistema debe ejecutar saltos (tecla C) con latencia ≤50 ms
- **RF4.2:** El sistema debe ejecutar planeo (tecla Space) con latencia ≤50 ms
- **RF4.3:** El sistema debe ejecutar bajada (flecha abajo) con latencia ≤50 ms
- **RF4.4:** Las acciones deben sincronizarse con la tasa de captura (30+ FPS)

#### RF5: Visualización y Debug
- **RF5.1:** El sistema debe mostrar en tiempo real los elementos detectados con rectángulos de color
- **RF5.2:** Debe visualizar el estado actual de Kong y los obstáculos cercanos
- **RF5.3:** Debe mostrar el puntaje y métricas en tiempo real

#### RF6: Configuración Centralizada
- **RF6.1:** Los parámetros de detección (rango HSV, área mínima/máxima, proporciones) deben estar centralizados en un archivo de configuración
- **RF6.2:** Los parámetros deben ser modificables sin recompilar el código

### 6.4 Requerimientos No Funcionales (Detalle RNF)

#### RNF1: Rendimiento
- **RNF1.1:** El pipeline completo (captura + detección + decisión + acción) debe ejecutarse en ≤100 ms (mantener tiempo real)
- **RNF1.2:** No debe generar lag perceptible en el emulador
- **RNF1.3:** El consumo de CPU no debe exceder el 40% en procesador de 4 núcleos

#### RNF2: Confiabilidad
- **RNF2.1:** El sistema debe mantener ejecución continua durante al menos 5 minutos sin fallos críticos
- **RNF2.2:** El sistema debe recuperarse automáticamente de frames corruptos o capturas fallidas
- **RNF2.3:** Las excepciones deben ser registradas en logs detallados

#### RNF3: Mantenibilidad
- **RNF3.1:** El código debe estar documentado con docstrings en toda función pública
- **RNF3.2:** La arquitectura debe ser modular con separación clara entre captura, detección, decisión y acción
- **RNF3.3:** Debe soportar fácil adición de nuevos detectores sin modificar código existente

#### RNF4: Portabilidad
- **RNF4.1:** El sistema debe funcionar en Windows 10/11
- **RNF4.2:** No debe depender de librerías propietarias
- **RNF4.3:** La configuración debe ser separable de la lógica para permitir adaptación a otros emuladores

#### RNF5: Usabilidad
- **RNF5.1:** La interfaz de usuario debe permitir activar/desactivar el bot con una tecla (Space)
- **RNF5.2:** Debe haber modo seguro donde se visualiza la detección sin ejecutar acciones
- **RNF5.3:** El sistema debe mostrar instrucciones claras al iniciar

#### RNF6: Seguridad y Ética
- **RNF6.1:** El sistema no modificará el cliente del juego
- **RNF6.2:** No accederá a memoria interna del juego
- **RNF6.3:** Operará únicamente en modo local (sin conexión online)
...
Antes de definir la arquitectura final, se evaluaron múltiples alternativas para abordar el problema de automatización visual.
...
| Criterio (Peso) | Redes Profundas | ML Simple | **Visión Clásica Híbrida** |
|-----------------|-----------------|-----------|------------------|
| Latencia (30%) | 1/10 (300ms) | 5/10 (50ms) | **10/10 (20ms)** |
| Hardware (25%) | 1/10 (GPU req) | 6/10 (GPU opt) | **10/10 (CPU)** |
| Interpretabilidad (20%) | 2/10 | 6/10 | **10/10** |
| Datos de entrenamiento (15%) | 1/10 (1000+) | 5/10 (100+) | **10/10 (0)** |
| Facilidad de debug (10%) | 2/10 | 6/10 | **10/10** |
| **Puntuación Final** | **1.6** | **5.3** | **✅ 9.9** |

**Conclusión:** El enfoque de visión clásica híbrida (color + template matching localizado) con reglas predefinidas es la opción óptima para este proyecto.
...
**Relación con alternativa seleccionada:** La arquitectura implementa el enfoque de visión clásica híbrida (color + template matching localizado) con reglas predefinidas, especificando cómo cada componente contribuye a satisfacer restricciones de tiempo real y mantenibilidad.
...
### 9.4 Monitoreo Operativo Local

El monitoreo actual se realiza en tiempo real por consola y ventanas OpenCV; no existe un servicio externo de observabilidad ni un archivo de log estructurado persistente por defecto.

Mecanismos implementados:

1. **Métricas de rendimiento en consola:** FPS, mínimo/máximo, tiempos por etapa (`captura`, `deteccion`, `estado`, `accion`) y tiempo de ciclo.
2. **Evaluación de detección en línea:** actualización manual de matriz de confusión por clase con exportación CSV en `./reportes`.
3. **Depuración visual:** overlay de elementos detectados y máscaras auxiliares para inspección de calibración.

Este esquema es suficiente para validación académica y pruebas iterativas, aunque no reemplaza un sistema de monitoreo de producción porque el proyecto no está orientado a despliegue productivo.
...
[1] Chrome Dino Bot - GitHub: https://github.com/search?q=chrome+dino+bot

[2] SerpentAI Framework - GitHub: https://github.com/SerpentAI/SerpentAI

[3] Bradski, G., & Kaehler, A. (2008). *Learning OpenCV: Computer Vision with the OpenCV Library*. O'Reilly Media.

[4] OpenCV Documentation - HSV Color Space: https://docs.opencv.org/master/df/d9d/tutorial_py_colorspaces.html

[5] MuMu Player Emulator: https://www.mumuplayer.com/

[6] PyAutoGUI Documentation: https://pyautogui.readthedocs.io/

[7] MSS (Multiple Screen Shots) Library: https://mss.readthedocs.io/

[8] Sutton, R. S., & Barto, A. G. (2018). *Reinforcement Learning: An Introduction* (2nd ed.). MIT Press.

[9] Goodfellow, I., Bengio, Y., & Courville, A. (2016). *Deep Learning*. MIT Press.

[10] Computer Vision Research - State of the Art: https://paperswithcode.com/area/computer-vision

[11] Banana Kong Game - Mobile Platform Game: https://www.banana-kong.com/

[12] Proyecto Ficha Propuesta: Especificación original del proyecto (documento interno)
